...
def opens(event=""):
    ...
    if path == "":
        ...
        if pos == "1.0" or count > 0:
            ...
            ask = tm.askquestion("MicroC IDE", "Do you want to save this text?")
            if ask == 'yes':
                save_as()
                text = path.read()
                editor.delete(1.0, END)
...
def new(event=""):
    ...
    if pos == "1.0":
        ...
        ask = tm.askyesnocancel("MicroC IDE", "Do you want to save this text?")
        if ask is True:
            get = save_as()
            if get is True:
                editor.delete(1.0, END)
...
def save_as():
    global path, count
    saveas = filedialog.asksaveasfile(initialfile="MicroC.c", defaultextension=".c",
                                      filetype=(("C Language", "*.c"), ("Text Document", "*.txt"), ("All Files", "*.*")))
    if saveas is None:
        return None
    else:
        saveas = saveas.name
...
def helps():
    tm.showinfo("About Us", "MicroC IDE Is Developed By Nikhil")
...
editor.bind("<Alt-c>", default_code)
...
bm.add_command(label="Build", command=build, accelerator="Ctrl+B")
bm.add_command(label="Run", command=run, accelerator="Ctrl+R")
bm.add_command(label="Build And Run", command=build_run, accelerator="Ctrl+M")
bm.add_separator()
bm.add_command(label="Default Code", command=default_code, accelerator="Alt+C")
