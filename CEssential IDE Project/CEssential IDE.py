...
def opens(event=""):
    ...
    if path == "":
        ...
        if pos == "1.0" or count > 0:
            ...
            ask = tm.askquestion("CEssential IDE", "Do you want to save this text?")
            if ask == 'yes':
                save_as()
                text = path.read()
                editor.delete(1.0, END)
...
def save_as():
    global path, count
    saveas = filedialog.asksaveasfile(initialfile="CEssential.c", defaultextension=".py",
                                      filetype=(("C Language", "*.c"), ("C++", "*.cpp"), ("All Files", "*.*")))
    if saveas is None:
        return None
    else:
        saveas = saveas.name
...
def helps():
    tm.showinfo("About Us", "CEssential IDE Is Developed By Nikhil")
...
def set_status():
    if status.get() == 0:
        ...
        tm.showinfo("CEssential IDE", "Restart CEssential IDE")
...
def build(event=""):
    if path != "":
        ...
        tm.showinfo("CEssential IDE", "Save the file to build")
        return
...
def run(event=""):
    if path != "":
        ...
        tm.showinfo("CEssential IDE", "Compile the program to run")
        return
...
bm.add_command(label="Build And Run", command=build_run, accelerator="Ctrl+Q")
