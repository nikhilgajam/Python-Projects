...
def update():
    ...
    if started:
        ...
        if seconds == 60:
            ...
            hours += 1
            minutes = 0
        time_display['text'] = "%02d:%02d:%02d" % (hours, minutes, seconds)
...
def reset():
    global hours, minutes, seconds, started
    started = False
    start_btn['text'] = "Start"
...
def lap():
    ...
    lap_display.insert(END, "%d.  %02d:%02d:%02d\n" % (lap_count, hours, minutes, seconds))
    lap_display['state'] = DISABLED
    lap_display.yview(END)  # Auto Scroll


# Time display label
time_display = Label(window, text="00:00:00", font=("Times New Roman", 50), fg="#e6e8eb", bg="#333333")
time_display.pack()
