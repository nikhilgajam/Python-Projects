...
def opens(event=""):
    ...
    if path == "":
        ...
        if pos == "1.0" or count > 0:
            ...
            count = 0
            root.title("Text Editor  –  " + os.path.basename(path))
        else:
            ask = tm.askquestion("Text Editor", "Do you want to save this text?")
            if ask == 'yes':
                save_as()
                text = path.read()
                editor.delete(1.0, END)
...
def new(event=""):
    ...
    if pos == "1.0":
        ...
        ask = tm.askyesnocancel("Text Editor", "Do you want to save this text?")
        if ask is True:
            get = save_as()
            if get is True:
                editor.delete(1.0, END)
...
def save_as():
    global count
    saveas = filedialog.asksaveasfile(initialfile="Text Editor.txt", defaultextension=".txt",
...
def set_status():
    if status.get() == 0:
        ...
        tm.showinfo("Text Editor", "Restart Text Editor")
